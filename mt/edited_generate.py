...
# This source code is based fairseq/generate.py 
"""
Perform edit and translate pre-processed data with a trained model.
"""
...
from fairseq_criterion import EditableTrainingCriterion
...
def main(args):
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        if args.edit_sample_index is not None:
            ...
            edited_model, success, _, complexity = criterion.get_edited_transformer(model, edit_sample, device, detach=True)
    ...
    if has_target:
        ...
        if args.edit_sample_index is not None:
            print('EditResult(success={}, complexity={})'.format(success, complexity))
    return scorer


def cli_main():
    parser = options.get_generation_parser()
    parser.add_argument('--edit-sample-index', type=int, metavar='D', default=None,
                        help='Index of edit sample to use (pass a dataset via criterion --edit-samples-path arg)')
    parser.add_argument('--moses-detokenizer', type=str, default=None)
    EditableTrainingCriterion.add_args(parser)
